...
class Agent:
    ...
    def make_one_transition(self, action):
        # print("inside make one transition")
        # next_states = []
    ...
    def MC_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            path = [(self.simulator.state, self.simulator.check_reward())]
            while True:
                act = self.default_policy(self.simulator.state)
                step = self.make_one_transition(act)
                if (step == None):
                    break
                else:
                    aa, rr = step
                    path.append((aa, rr))
            returns = 0
            for tt in range(len(path)-1, -1, -1):
                # print(tt)
                ss, rr = path[tt]
                returns = returns + rr*DISCOUNT
                self.N_MC[ss] = self.N_MC[ss] + 1
                self.S_MC[ss] = self.S_MC[ss] + rr*DISCOUNT
                # self.MC_values[ss] = self.MC_values[ss] + self.S_MC[ss]/self.N_MC[ss]
                self.MC_values[ss] = self.MC_values[ss] + ((returns - self.MC_values[ss])/self.N_MC[ss])
    ...
    def TD_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            while not done:
                ...
                reward = 0
                next_action = 0
                tt = self.make_one_transition(action)
                if (tt != None):
                    next_state, reward = tt
                next_val = 0
                if (next_state != None):
                    next_val = self.TD_values[next_state]
                update = reward + DISCOUNT * next_val - self.TD_values[curr_state]
                self.TD_values[curr_state] = self.TD_values[curr_state] + self.alpha(self.N_TD[curr_state]) * update
                self.N_TD[curr_state] = self.N_TD[curr_state] + 1
                curr_state = next_state
                if (curr_state == None):
                    done = True
        self.TD_values[(1, 0, 0)] = -1
        self.TD_values[(0, 0, 0)] = 1
    ...
    def Q_run(self, num_simulation, tester=False, epsilon=0.4):
        ...
        for simulation in range(num_simulation):
            ...
            while not done:
                ...
                next_val = 0
                if (next_state != None):
                    next_val = self.Q_values[next_state][next_action]
                update = reward + DISCOUNT * next_val - self.Q_values[curr_state][action]
                self.Q_values[curr_state][action] = self.Q_values[curr_state][action] + self.alpha(self.N_Q[curr_state][action]) * update
                self.N_Q[curr_state][action] = self.N_Q[curr_state][action] + 1
                curr_state = next_state
                if (self.simulator.game_over() == True):
                    done = True

        self.Q_values[(1, 0, 0)][0] = -1
        self.Q_values[(0, 0, 0)][0] = 1
        self.Q_values[(1, 0, 0)][1] = -1
        self.Q_values[(0, 0, 0)][1] = 1
